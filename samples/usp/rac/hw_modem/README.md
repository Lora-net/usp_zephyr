...
| `CONFIG_USE_FLRC_BURST`              | `n`           | FLRC burst commands + 20KB buffer |
...
- FLRC burst commands (`0xB0`–`0xB2`) and the 20KB payload buffer are compiled only when this option is enabled
- Requires a LR20XX radio (FLRC burst support)
- Set `CONFIG_USE_FLRC_BURST=y` in `prj.conf` to enable
...
### FLRC burst (requires `CONFIG_USE_FLRC_BURST=y`)
